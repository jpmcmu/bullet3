...
void	btAxisSweep3Internal<BP_FP_INT_TYPE>::destroyProxy(btBroadphaseProxy* proxy,btDispatcher* dispatcher)
{
	...
	removeHandle(handle->m_handle, dispatcher);
}
...
void	btAxisSweep3Internal<BP_FP_INT_TYPE>::setAabb(btBroadphaseProxy* proxy,const btVector3& aabbMin,const btVector3& aabbMax,btDispatcher* dispatcher)
{
	...
	updateHandle(handle->m_handle, aabbMin, aabbMax,dispatcher);
	if (m_raycastAccelerator)
...
#else
	btVector3 vf = (point - m_worldAabbMin) * m_quantize;
	float v[3] = {(float)vf[0],(float)vf[1],(float)vf[2]}; 
...
BP_FP_INT_TYPE btAxisSweep3Internal<BP_FP_INT_TYPE>::addHandle(const btVector3& aabbMin,const btVector3& aabbMax, void* pOwner, int collisionFilterGroup, int collisionFilterMask,btDispatcher* dispatcher)
{
	...
	{
		pHandle->m_uniqueId = generateUID(pOwner);
	}
